    ...
    */
    ...
    document.addEventListener("keydown", function(e) {
        ...
        if (timestamp && !timeTaken && !loading && choices.length === battleNo - 1) {
            ...
            e.preventDefault();
        } else if (timeTaken && choices.length === battleNo - 1) { /** If sorting has ended. */ } else { // If sorting hasn't started yet.
            switch (e.which) {
                case 13:
                    start();
                    break;
...
function selectMuse() {
    ...
    document.querySelector('#muse').onclick = ""; //diable all buttons after selecting
    document.querySelector('#aqours').onclick = ""; 
    document.querySelector('#nijigaku').onclick = ""; 
